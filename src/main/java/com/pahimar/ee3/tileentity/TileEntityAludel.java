...
public class TileEntityAludel extends TileEntityEE implements ISidedInventory {
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        return side == ForgeDirection.DOWN.ordinal() ? new int[] { FUEL_INVENTORY_INDEX, OUTPUT_INVENTORY_INDEX }
    ...
    public Packet getDescriptionPacket() {
        return PacketHandler.INSTANCE
            .getPacketFrom(new MessageTileEntityAludel(this, inventory[OUTPUT_INVENTORY_INDEX]));
    }
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (this.deviceCookTime == 0 && this.canInfuse()) {
                this.fuelBurnTime = this.deviceCookTime = TileEntityFurnace
                    .getItemBurnTime(this.inventory[FUEL_INVENTORY_INDEX]);
                ...
                if (this.deviceCookTime > 0) {
                    ...
                    if (this.inventory[FUEL_INVENTORY_INDEX] != null) {
                        ...
                        if (this.inventory[FUEL_INVENTORY_INDEX].stackSize == 0) {
                            this.inventory[FUEL_INVENTORY_INDEX] = this.inventory[FUEL_INVENTORY_INDEX].getItem()
    ...
    private boolean canInfuse() {
        if (!hasGlassBell || inventory[INPUT_INVENTORY_INDEX] == null || inventory[DUST_INVENTORY_INDEX] == null) {
            ...
            ItemStack infusedItemStack = AludelRecipeManager.getInstance()
                .getResult(inventory[INPUT_INVENTORY_INDEX], inventory[DUST_INVENTORY_INDEX]);
            ...
            if (inventory[OUTPUT_INVENTORY_INDEX] == null) {
                ...
                int mergedOutputStackSize = this.inventory[OUTPUT_INVENTORY_INDEX].stackSize
    ...
    public void infuseItem() {
        if (this.canInfuse()) {
            RecipeAludel recipe = AludelRecipeManager.getInstance()
                .getRecipe(inventory[INPUT_INVENTORY_INDEX], inventory[DUST_INVENTORY_INDEX]);

            if (this.inventory[OUTPUT_INVENTORY_INDEX] == null) {
                this.inventory[OUTPUT_INVENTORY_INDEX] = recipe.getRecipeOutput()
                    .copy();
