...
public class TileEntityTransmutationTablet extends TileEntityEE implements ISidedInventory {
                ...
                for (int i = 0; i < STONE_INDEX; i++) {
                    ...
                    if (stackInSlot != null && EnergyValueRegistryProxy.hasEnergyValue(stackInSlot)) {
                        this.storedEnergy = new EnergyValue(
                            this.storedEnergy.getValue() + EnergyValueRegistryProxy.getEnergyValue(stackInSlot)
                                .getValue());
                        decrStackSize(i, 1);
                    }
                }
    ...
    public void updateEnergyValueFromInventory() {
        ...
        for (int i = 0; i <= STONE_INDEX; i++) {
            if (inventory[i] != null && EnergyValueRegistryProxy.hasEnergyValue(inventory[i])) {
                newEnergyValue += EnergyValueRegistryProxy.getEnergyValueForStack(inventory[i])
                    .getValue();
            }
        }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return AxisAlignedBB
            .getBoundingBox(xCoord - 1.5d, yCoord - 1, zCoord - 1.5d, xCoord + 1.5d, yCoord + 1, zCoord + 1.5d);
    }
    ...
    public boolean isStructureValid() {
        ...
        return ((worldObj.getBlock(xCoord - 1, yCoord, zCoord - 1) instanceof BlockAshInfusedStoneSlab
            ...
            && (worldObj.getBlock(xCoord + 1, yCoord, zCoord - 1) instanceof BlockAshInfusedStoneSlab
                && worldObj.getBlockMetadata(xCoord + 1, yCoord, zCoord - 1) == 3)
            && (worldObj.getBlock(xCoord - 1, yCoord, zCoord) instanceof BlockAshInfusedStoneSlab
