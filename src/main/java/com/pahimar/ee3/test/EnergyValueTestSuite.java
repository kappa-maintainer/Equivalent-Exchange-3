...
public class EnergyValueTestSuite {
    ...
    private static final Marker FAILURE_WRONG_VALUE_MARKER = MarkerManager
        .getMarker("EE3_TEST_FAILURE_WRONG_VALUE", FAILURE_MARKER);
    private static final Marker FAILURE_NO_VALUE_MARKER = MarkerManager
        .getMarker("EE3_TEST_FAILURE_NO_VALUE", FAILURE_MARKER);
        ...
        for (WrappedStack wrappedStack : testSuiteValueMap.keySet()) {
            ...
            if (actualValue == null && expectedValue == null) {
                ...
                if (actualValue.equals(expectedValue)) {
                    ...
                    // Failure - anticipated that a specific value would be found and while a value was found it was not
                    // the anticipated one
                    failureMessagesWrongValue.add(
                        String.format(
                            "FAILURE: Object '%s' did not have the expected energy value [Expected (%s), Found (%s)]",
                            wrappedStack,
                            expectedValue,
