    ...
    public void registerIcons(IIconRegister iconRegister) {
        ...
        for (int i = 0; i < ALCHEMICAL_BAG_ICONS.length; i++) {
            icons[i] = iconRegister
                .registerIcon(Textures.RESOURCE_PREFIX + Names.Items.ALCHEMICAL_BAG + "." + ALCHEMICAL_BAG_ICONS[i]);
        }
    }
            ...
            for (int i = 0; i < entityPlayer.inventory.getSizeInventory(); i++) {
                if (entityPlayer.inventory.getStackInSlot(i) != null) {
                    ...
                    if (itemStack1.getItem() instanceof ItemAlchemicalBag) {
                        if (NBTHelper.hasUUID(itemStack1) && NBTHelper.getUUID(itemStack)
                            .equals(NBTHelper.getUUID(itemStack1))) {
