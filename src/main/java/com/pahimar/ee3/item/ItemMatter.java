...
public class ItemMatter extends ItemEE {
    ...
    public String getUnlocalizedName(ItemStack itemStack) {
        return String.format(
            ...
            Names.Items.MATTER,
            Names.Items.MATTER_SUBTYPES[MathHelper
                .clamp_int(itemStack.getItemDamage(), 0, Names.Items.MATTER_SUBTYPES.length - 1)]);
    }
    ...
    public void registerIcons(IIconRegister iconRegister) {
        ...
        for (int i = 0; i < Names.Items.MATTER_SUBTYPES.length; i++) {
            icons[i] = iconRegister
                .registerIcon(Textures.RESOURCE_PREFIX + Names.Items.MATTER + Names.Items.MATTER_SUBTYPES[i]);
        }
    }
}
