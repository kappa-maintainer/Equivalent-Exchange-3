...
public class DrawBlockHighlightEventHandler {
            ...
            if (toolMode == ToolMode.STANDARD) {
                ...
                if (event.target.sideHit == ForgeDirection.NORTH.ordinal()
                    ...
                    drawSelectionBox(
                        ...
                        0,
                        event.partialTicks);
                } else if (event.target.sideHit == ForgeDirection.EAST.ordinal()
    ...
    private void drawAlchemyArrayOverlay(DrawBlockHighlightEvent event) {
        ...
        TileEntity tileEntity = event.player.worldObj
    ...
    private boolean isValidForArray(World world, int x, int y, int z, int sideHit) {
        ...
        return world.isSideSolid(x, y, z, side) && ((side == ForgeDirection.DOWN && world.getBlock(x, y - 1, z)
            .isReplaceable(world, x, y, z)) || (side == ForgeDirection.UP
                && world.getBlock(x, y + 1, z)
                    .isReplaceable(world, x, y, z))
            || (side == ForgeDirection.NORTH && world.getBlock(x, y, z - 1)
                .isReplaceable(world, x, y, z))
            || (side == ForgeDirection.SOUTH && world.getBlock(x, y, z + 1)
                .isReplaceable(world, x, y, z))
            || (side == ForgeDirection.WEST && world.getBlock(x - 1, y, z)
                .isReplaceable(world, x, y, z))
            || (side == ForgeDirection.EAST && world.getBlock(x + 1, y, z)
                .isReplaceable(world, x, y, z)));
    }

    private void drawSelectionBox(RenderGlobal context, EntityPlayer entityPlayer, MovingObjectPosition rayTrace, int i,
        float partialTicks) {
